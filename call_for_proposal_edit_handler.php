...
if (!$id || !$title || !$description || !$start_date || !$end_date) {
    ...
    header('Location: call_for_proposal_edit.php?id=' . urlencode((string) $id));
    exit();
}
...
try {
    ...
    if (isset($_FILES['pdf']) && $_FILES['pdf']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!move_uploaded_file($_FILES['pdf']['tmp_name'], $destination_path)) {
            $pdo->rollBack();
    ...
    if (isset($_FILES['application_documents_zip']) && $_FILES['application_documents_zip']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if (!move_uploaded_file($_FILES['application_documents_zip']['tmp_name'], $zip_destination_path)) {
            $pdo->rollBack();
            $_SESSION['call_for_proposal_form_error'] = 'Errore durante il caricamento dello ZIP dei documenti.';
            header('Location: call_for_proposal_edit.php?id=' . urlencode((string) $id));
            exit();
    ...
    $existingAssignmentsStmt = $pdo->prepare('SELECT evaluator_user_id FROM call_for_proposal_evaluator WHERE call_for_proposal_id = :call_for_proposal_id');
    ...
    if ($evaluatorIdsToRemove !== []) {
        ...
        $submittedOrRevisedStmt = $pdo->prepare("SELECT DISTINCT e.evaluator_id FROM evaluation e JOIN application a ON a.id = e.application_id WHERE a.call_for_proposal_id = ? AND e.status IN ('SUBMITTED', 'REVISED') AND e.evaluator_id IN (" . $placeholders . ')');
        ...
        if ($lockedEvaluatorIds !== []) {
            $pdo->rollBack();
            $_SESSION['call_for_proposal_form_error'] = 'Non puoi rimuovere valutatori che hanno gia inviato una valutazione per questo bando.';
            header('Location: call_for_proposal_edit.php?id=' . urlencode((string) $id));
            exit();
        }
    }
    ...
    foreach ($evaluatorUserIds as $evaluatorUserId) {
        $insertAssignmentStmt->execute([':call_for_proposal_id' => $id, ':evaluator_user_id' => $evaluatorUserId]);
    }
