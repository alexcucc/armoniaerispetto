...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: supervisor_add.php');
    exit();
}
...
try {
    $stmt = $pdo->prepare("INSERT INTO supervisor (user_id) VALUES (:user_id)");
    $stmt->execute([':user_id' => $user_id]);

    $roleStmt = $pdo->prepare(
        "INSERT INTO user_role (user_id, role_id)
         VALUES (:user_id, (SELECT id FROM role WHERE name = 'Supervisor'))"
    );
